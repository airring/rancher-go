...
func RefreshToken(tokenString string) (string, error) {
	...
	if err != nil {
		beego.Error("generate new fresh json web token failed !! error :", err)
		return "", err
	}
	return tokenStr, err
}
...
func ValidateToken(tokenString string) error {
	...
	if claims, ok := token.Claims.(*MyCustomClaims); ok && token.Valid {
		beego.Info("%v %v", claims.User, claims.StandardClaims.ExpiresAt)
		beego.Info("token will be expired at ", time.Unix(claims.StandardClaims.ExpiresAt, 0))
	} else {
		beego.Error("validate tokenString failed !!!", err)
		return err
	}
	return nil
}
...
func GenerateToken(info *User, expiredSeconds int) (tokenString string, err error) {
	...
	beego.Info("token will be expired at ", time.Unix(expireAt, 0))
